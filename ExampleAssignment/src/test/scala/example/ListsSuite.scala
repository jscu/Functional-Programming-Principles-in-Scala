 ...
 */
 class ListsSuite {
  ...
  @Test def `sum of a negative numbers`: Unit = {
  ...
  @Test def `sum of list with repeated elements`: Unit = {
    assert(sum(List(1, 2, 1, 2, 3)) == 9)
  }
  ...
  @Test def `max of list with repeated elements`: Unit = {
    assert(max(List(3, 7, 3, 7, 2)) == 7)
  }
